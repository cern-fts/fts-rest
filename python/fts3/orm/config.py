...
class Se(Base):
	...
	source_on = relation('LinkConfig', backref = 'source_se',
						 primaryjoin = and_(LinkConfig.source == name, LinkConfig.destination != '*'),
						 foreign_keys = (name), uselist = True,
						 lazy = 'dynamic')
	
	destination_on = relation('LinkConfig', backref = 'destination_se',
							  primaryjoin = and_(LinkConfig.destination == name, LinkConfig.source != '*'),
							  foreign_keys = (name), uselist = True,
							  lazy = 'dynamic')
	
	standalone_source = relation('LinkConfig', backref = None,
								 primaryjoin = and_(LinkConfig.source == name, LinkConfig.destination == '*'),
								 foreign_keys = (name),
								 lazy = 'dynamic')
	
	standalone_destination = relation('LinkConfig', backref = None,
									  primaryjoin = and_(LinkConfig.destination == name, LinkConfig.source == '*'),
									  foreign_keys = (name),
									  lazy = 'dynamic')
...
class ShareConfig(Base):
	...
	__table_args__ = (ForeignKeyConstraint(['source', 'destination'],
										   [LinkConfig.source, LinkConfig.destination]),)
